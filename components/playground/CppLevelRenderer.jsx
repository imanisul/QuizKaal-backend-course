...
import { 
  ArrowLeft, Sparkles, Check, Mountain, Code2, Cpu, 
...
const ICONS = {
  Mountain, Code2, Cpu, Flame, Layers, Puzzle
};

export default function CppLevelRenderer({ levelData }) {
  ...
  const MascotIcon = ICONS[levelData.intro.mascotIcon] || Mountain;

  const handleValidateCode = (currentCode) => {
    playClick();
    setIsCompiling(true);
    setTerminalOutput('Compiling main.cpp...\nRunning...\n');
  ...
  const handleQuizAnswer = (answer) => {
    if (answer === levelData.quiz.correctAnswer) {
      if (!codeSuccess) {
        setCodeError('You must successfully compile the C++ code before taking the quiz!');
        playError();
        return;
      }
      playSuccess();
      ...
      setTimeout(() => {
        ...
        markLevelComplete(`cpp-${levelData.id}`);
        playCoin();
      }, 1500);
    } else {
      playError();
  ...
  return (
    <div className="flex-1 w-full bg-[#0a0a0a] text-slate-200 flex flex-col font-ui relative">
      ...
      <div className="h-16 border-b border-white/10 bg-black/50 backdrop-blur-md px-6 flex items-center justify-between sticky top-0 z-50 shadow-sm">
        <Link href="/playground/cpp-mountains" className="flex items-center gap-2 text-slate-400 hover:text-white font-bold transition-colors">
          <ArrowLeft size={16} /> Return to Mountain Map
        </Link>
        <div className="font-black text-slate-200 flex items-center gap-2 tracking-widest uppercase">
          <MascotIcon size={18} className="text-blue-500" /> {levelData.title}
        </div>
      </div>
      ...
      <div className="flex-1 w-full max-w-5xl mx-auto flex flex-col relative py-12 px-6 gap-16">
        ...
        <section className="text-center flex flex-col items-center">
          <motion.div 
            ...
            className="w-32 h-32 bg-gradient-to-br from-slate-800 to-slate-900 rounded-[40px] flex items-center justify-center mb-8 shadow-2xl shadow-blue-500/10 border-4 border-slate-700 rotate-3"
          >
            <MascotIcon className="w-16 h-16 text-blue-400" />
          </motion.div>
        ...
        {/* 2. C++ THEORY & BEST PRACTICES */}
        <section className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="bg-slate-900 p-8 rounded-3xl shadow-xl border border-white/10">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-10 h-10 bg-blue-900/50 rounded-xl flex items-center justify-center border border-blue-500/30">
          ...
          <div className="p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-white/5 bg-black/20">
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-blue-500/20 text-blue-300 rounded-full text-xs font-bold mb-3 border border-blue-500/30">
                <Code2 size={14} /> Interactive C++ Compiler
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-white/10 h-[500px]">
            ...
            <div className="flex flex-col relative h-[500px]">
              ...
              <div className="absolute bottom-4 left-4 z-10 flex gap-2">
                <button 
                  onClick={() => {
                    const editorValue = document.querySelector('.monaco-editor')?.innerText || levelData.codeChallenge.initialCode;
                    // Note: Since CodeEditorPanel doesn't easily expose value outside, we trigger it via its internal submit, or we pass state up.
                    // For this architecture, CodeEditorPanel's onSubmit passes the code up.
                    // The button inside CodeEditorPanel handles submission. We'll just rely on the editor's built in submit button.
                  }}
                  className="hidden"
                />
              </div>
            ...
            <div className="flex flex-col bg-[#0d0d0d]">
              ...
              <div className="flex-1 p-6 font-mono text-sm overflow-y-auto whitespace-pre-wrap">
                {isCompiling ? (
                  <span className="text-slate-400 animate-pulse">Compiling main.cpp...</span>
                ) : terminalOutput ? (
        ...
        <section className="bg-slate-900 p-10 rounded-3xl shadow-xl border border-white/10">
          <div className="flex items-center gap-3 mb-8 border-b border-white/5 pb-6">
            ...
            <div>
              ...
              <p className="text-slate-400 font-medium">Real C++ & DSA questions asked in technical interviews.</p>
            </div>
          </div>
        ...
        <section className="bg-gradient-to-br from-blue-900 to-indigo-900 p-10 rounded-3xl shadow-2xl text-white relative overflow-hidden mb-20 border border-blue-500/30">
          <div className="absolute top-0 right-0 w-64 h-64 bg-white/5 rounded-full blur-3xl -translate-y-1/2 translate-x-1/4" />
          
          <h2 className="text-sm font-black text-blue-300 uppercase tracking-widest mb-4">Final Knowledge Check</h2>
        ...
        {showRewardModal && (
          ...
          >
            ...
            <motion.div 
              ...
              className="bg-slate-900 rounded-3xl p-12 max-w-2xl w-full text-center relative overflow-hidden shadow-[0_0_50px_rgba(59,130,246,0.3)] border border-blue-500/30"
            >
              <div className="absolute top-0 left-0 w-full h-4 bg-gradient-to-r from-blue-400 to-indigo-600" />
              
              <div className="w-24 h-24 mx-auto bg-blue-900/50 border border-blue-500/50 rounded-full flex items-center justify-center mb-6 shadow-[0_0_30px_rgba(59,130,246,0.5)]">
                <Sparkles className="w-12 h-12 text-blue-400" />
              </div>

              <h1 className="text-4xl font-black text-white mb-4">Level Complete!</h1>
              <p className="text-lg text-slate-400 font-medium mb-10">You have mastered the concepts and passed the DSA interview check!</p>
              ...
              <Link href="/playground/cpp-mountains" className="inline-flex items-center gap-2 px-8 py-4 bg-blue-600 text-white font-black text-lg rounded-full hover:bg-blue-500 active:scale-95 transition-all shadow-[0_0_20px_rgba(59,130,246,0.5)]">
                <Mountain size={20} /> Return to Map
              </Link>
            </motion.div>
          </motion.div>
        )}
