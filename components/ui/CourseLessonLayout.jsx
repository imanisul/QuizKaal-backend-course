 ...
 */
export default function CourseLessonLayout({ lesson, courseId, courseName, allLessonIds = [], backLink, children, sidebar, toc }) {
  ...
  return (
    <div className="relative w-full min-h-screen bg-[#060608] selection:bg-purple-500/30 font-ui overflow-hidden">
      ...
      <div className={`mx-auto px-4 sm:px-8 relative z-10 pt-12 pb-24 ${sidebar ? 'max-w-[1400px] grid grid-cols-1 xl:grid-cols-[240px_minmax(0,1fr)_240px] gap-10' : 'max-w-5xl'}`}>
        
        {/* LEFT SIDEBAR (Optional) */}
        {sidebar && (
          <aside className="hidden xl:block pb-32 overflow-y-auto border-r border-white/10 pr-2">
             {sidebar}
          </aside>
        )}
        ...
        {/* RIGHT TOC (Optional) */}
        {toc && (
          <aside className="hidden xl:block pb-32 overflow-y-auto pl-6">
            {toc}
          </aside>
        )}
      </div>
