...
<BehindTheScenes title="Deep Dive: Container Runtime (containerd)">
The Docker Daemon doesn't actually run containers itself anymore. It delegates that job to **containerd** and **runc**. 

When you run a container, Docker passes the image and configuration to `containerd`, which then calls `runc` to create the Linux namespaces and cgroups, finally starting the process. This modular architecture makes Docker much more stable and standardized.
</BehindTheScenes>
