...
export function generateMetadata({ params }) {
  ...
  const title = `${formatted} - AI Prompt Engineering | QuizKaal`;
  const url = `https://quizkaal.in/ai-prompt-engineering/${params.lesson || params.slug || params.id || params.courseId}`;

  return generateSEOMetadata({
    title,
    description: `Master ${formatted} in the AI Prompt Engineering course on QuizKaal Learn.`,
    url,
    type: "article",
    keywords: ["${formatted}", "AI Prompt Engineering", "quizkaal learn"]
  });
}
...
export default function DynamicPageWrapper({ params }) {
  ...
  const articleSchema = generateSchema("TechArticle", {
    title: `${formatted} - AI Prompt Engineering`,
    description: `Learn ${formatted} on QuizKaal`,
    url: `https://quizkaal.in/ai-prompt-engineering/${params.lesson || params.slug || params.id || params.courseId}`
  });
