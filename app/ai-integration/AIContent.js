          ...
          <StaggerItem>
            <div className="glass-card p-2">
              <QAAccordion questions={[
                ...
                { q: "What is Retrieval-Augmented Generation (RAG)?", a: "RAG is a technique where you retrieve relevant factual context from a Vector Database based on the user's query, and then inject that context into the LLM's prompt. This allows the LLM to answer questions about private or recent data it was never trained on." },
                { q: "How do you handle LLM hallucinations in a production application?", a: "You can mitigate hallucinations by using RAG to ground the model in facts, enforcing strict JSON output schemas (like OpenAI Structured Outputs), setting a lower temperature parameter (e.g., 0.1), and instructing the model to say 'I don't know' if the context does not contain the answer." },
                { q: "What is the primary difference between a System Prompt and a User Prompt?", a: "The System Prompt establishes the core persona, rules, boundaries, and formatting instructions for the AI (e.g., 'You are a helpful banking assistant. Only output JSON.'). The User Prompt is the actual query or task provided by the end-user. The model generally gives higher authoritative weight to the System Prompt." },
                { q: "Why is the chunking strategy critical when building a Vector Database for RAG?", a: "If your chunks are too large (e.g., an entire book chapter), the retrieval context gets diluted with irrelevant information. If the chunks are too small (e.g., a single sentence), the LLM loses the broader context needed to synthesize an accurate answer. Finding the 'Goldilocks' chunk size with overlap is essential for high-quality retrieval." },
                { q: "What is Semantic Chunking?", a: "Instead of arbitrarily splitting text every 500 tokens (which might cut a sentence in half), Semantic Chunking uses NLP techniques to split documents by logical boundaries—like paragraphs, headers, or complete thoughts. This preserves the contextual integrity of the data before it is embedded." },
                { q: "How do you monitor and evaluate LLM performance in production?", a: "Traditional metrics like latency and error rates aren't enough. You must use specialized LLM observability platforms (like LangSmith or Helicone) to track token usage, cost-per-request, prompt drift, and user feedback (thumbs up/down). You also use 'LLM-as-a-judge' automated evals in your CI/CD pipeline." }
              ]} />
            </div>
          </StaggerItem>
        </section>
