  ...
  return (
    <>
      ...
      <section id="hero" className="relative pb-24 border-b border-white/[0.06] mb-16 scroll-mt-24">
        <CourseHeader 
          title="CI/CD Pipeline: Laptop to Production"
          description="Writing code is only 50% of the job. How does that code reliably get from your laptop, through rigorous automated testing, packaged into Docker, and deployed into a Kubernetes cluster serving millions? Welcome to Continuous Integration and Continuous Deployment."
          icon={GitBranch}
          completedCount={0}
          totalLessons={5}
          nextLessonTitle="The Deployment Journey"
          nextLessonPath="#simulator"
          themeColor="from-blue-400 to-green-400"
          bgGlow="from-blue-400/20 to-green-400/20"
        />
      </section>
          ...
          <StaggerItem>
            <div className="glass-card p-2">
              <QAAccordion questions={[
                ...
                { q: "What is the exact difference between Continuous Integration (CI) and Continuous Deployment (CD)?", a: "CI is the automated process of building and testing code every time a developer commits (e.g., running Jest tests on GitHub Actions). CD automatically takes that validated code and deploys it to a production environment without human intervention." },
                { q: "How do you securely manage secrets (like API keys or Database passwords) in a CI/CD pipeline?", a: "Secrets should never be hardcoded or checked into Git. They should be injected at runtime using secure secret managers like AWS Secrets Manager, HashiCorp Vault, or GitHub Actions Secrets. The pipeline pulls the secret securely and passes it as an environment variable to the container." },
                { q: "What is GitOps?", a: "GitOps is an operational framework where Git is the single source of truth for your infrastructure. Tools like ArgoCD constantly monitor the Git repository, and if the live Kubernetes cluster deviates from what is declared in the Git repo, the tool automatically syncs the cluster back to the desired state." },
                { q: "Explain the concept of 'Immutable Infrastructure'.", a: "Instead of SSHing into a live server to run `apt-get update` or modify configuration files (mutable), immutable infrastructure means you never change a running server. If an update is needed, you destroy the old server and spin up a brand new one from a freshly baked image." },
                { q: "What is a Canary Release?", a: "A Canary Release deploys the new version of the application to a tiny subset of users (e.g., 5%). You monitor error rates and performance for that 5%. If everything is healthy, you gradually ramp it up to 100%. If errors spike, you instantly roll back." }
              ]} />
            </div>
          </StaggerItem>
        </section>
